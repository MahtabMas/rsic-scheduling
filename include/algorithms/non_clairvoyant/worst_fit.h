...
class WorstFit_dimensional : public BaseAlgorithm{
public:
    std::vector<int64_t> active_servers_idx;
    int64_t cur_time;

    WorstFit_dimensional(const std::vector<int64_t>& _capacity) : BaseAlgorithm(_capacity), cur_time(0){}
    ...
    void add_job(Job_dimensional& j) {
        ...
        int64_t max_remain_capacity_idx = -1;
        ...
        if (max_remain_capacity_idx != -1) {
            int64_t finishing_time = all_servers[max_remain_capacity_idx].start_time + all_servers[max_remain_capacity_idx].duration();
            ...
            for (size_t dimension = 0; dimension < j.size.size(); ++dimension) {
                ...
                all_servers[max_remain_capacity_idx].wasted_space[dimension] += new_wasted_space;
            }
