...
class BestFit_FirstFit {
public:
    BestFit_dimensional FFShort;  // One BestFit instances
    FirstFit_dimensional FFLong;  // One FirstFit instances
    ...
    BestFit_FirstFit(const std::vector<int64_t>& _capacity, int64_t _threshold): capacity(_capacity), threshold(_threshold), FFShort(_capacity), FFLong(_capacity) {}
    ...
    void print_job_count() const{
        std::cout << "Number of jobs processed by BestFit Short: " << short_count << std::endl;
        std::cout << "Number of jobs processed by FirstFit Long: " << long_count << std::endl;
    ...
    void print_servers() {
        ...
        FFShort.print_servers("BestFit_Short");
        std::cout << "************** LONG DURATION JOB SERVERS ******************" << std::endl;
        FFLong.print_servers("FirstFit_Long");
    }
